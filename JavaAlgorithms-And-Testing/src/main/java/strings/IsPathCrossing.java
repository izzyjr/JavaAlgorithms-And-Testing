...
public class IsPathCrossing {
    ...
    public static boolean isPathCrossing(String path) {
        ...
        for (int i = 0; i < path.length(); i++) {
            if (path.charAt(i) == 'N') {
                currently[0] += 1;
                if (northOrSouth(coordinates, currently)) {
                    return true;
                }
                ...
                if (coordinates.containsKey(currently[0])) {
                    if (coordinates.get(currently[0]).contains(currently[1])) {
                        ...
                        coordinates.get(currently[0]).add(currently[1]);
                    }
                }
    ...
    private static boolean northOrSouth(Map<Integer, ArrayList<Integer>> coordinates, int[] currently) {
        if (coordinates.containsKey(currently[0])) {
            if (coordinates.get(currently[0]).contains(currently[1])) {
                return true;
            } else {
